import { SettingsSchema, type Glyph, type Metrics, type Settings } from '@/types'
import { cropPixels, getBounds, packPixel, translatePixels, type Pixels } from '@/utils/pixel'
...
import { glyphIsEmpty, parseFont, serializeFont, type GfxGlyph } from '@/utils/font'
...
export const useFont = defineStore('font', () => {
    ...
    const addGlyph = (data: Partial<Glyph> & Required<Pick<Glyph, 'code'>>) => {
      const { code, pixels = new Set(), bearing = { left: 0, right: 0 }, version = 0 } = data
      const glyph = {
        code,
        pixels,
        bearing,
